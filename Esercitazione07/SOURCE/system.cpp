...
void System :: move(int i){ // Propose a MC move for particle i
  if(_sim_type == 3){ //Gibbs sampler for Ising
    ...
    double JHsum_spin, p_up;
    JHsum_spin = _J*(_particle(this->pbc(i-1)).getspin() + _particle(this->pbc(i+1)).getspin() ) + _H;  // Compute the effective local field acting on spin i 
    p_up = 1./(1+exp(-2*_beta*JHsum_spin)); // Compute the conditional probability of assigning spin +1  
    if(_rnd.Rannyu()< p_up) // Sample the new spin value from the conditional distribution 
      _particle(i).setspin(1);
    else 
      _particle(i).setspin(-1);
    _naccepted++; // In Gibbs sampling, the proposed spin update is always accepted
  } else {           // M(RT)^2
...
void System :: initialize_velocities(){ 
  if(_restart and _sim_type==0){
    ...
    sumv2 /= double(_npart); //media sumv2
...
void System :: initialize_properties(){ // Initialize data members used for measurement of properties
  ...
  _measure_penergy  = false; //Defining which properties will be measured (mette a false la variabile booleana = non le misura)
  ...
  if (input.is_open()){
    while ( !input.eof() ){
      ...
      if( property == "POTENTIAL_ENERGY" ){
        ...
        index_property++;
        _vtail = ((8*M_PI*_rho)/3)*(1./(3*pow(_r_cut,9))-1./(pow(_r_cut,3))); // TO BE FIXED IN EXERCISE 7 tail correction for potential energy
        ...
        index_property++;
        _ptail = 32*M_PI*_rho*(1./(9*pow(_r_cut,9))-1./(6*pow(_r_cut,3)))  ; // TO BE FIXED IN EXERCISE 7 tail correction for pressure
...
void System :: read_configuration(){
  ...
  if(_restart and _sim_type > 1){ 
    cout<<"Parto da configurazione di spin precedente"<<endl;
    int spin;
...
void System :: measure(){ // Measure properties
  ...
  //armadillo vector to build the histogram to compute gofr
  vec histo;
  if(_measure_gofr and _n_bins!=0)
    histo.zeros(_n_bins);
  ...
  double virial=0.0; //variable to store virial used to compute pressure
  double chi_temp=0.0; // temporary accumulator for susceptibility
  double H_temp=0.0;  // temporary accumulator for H used to compute c_v
      ...
      for (int j=i+1; j<_npart; j++){
        ...
        if (_measure_gofr){
          for (int k=0; k<_n_bins; k++){
            if (dr>k*_bin_size and dr<(k+1)*_bin_size) // Check whether the distance between the particle pair falls within the radial shell
                histo[k] = histo[k]+2;  //if "f" condition is satisfied-->adds +2. This counts both ordered pairs (i,j) and (j,i) in the corresponding bin
          }
        }
        // POTENTIAL ENERGY
        if(dr < _r_cut){
          ...
          if (_measure_pressure)
            virial += 48.0/pow(dr,12)-24.0/pow(dr,6);
...
//Changed for this exercise to have the correct estimation of pressure if Verlet's simulation IS NOT performed 
  if(_measure_pressure){

      if(_sim_type ==0){
        virial = 1/(3*_volume)*virial + _ptail*_rho;
        _measurement(_index_pressure) = _rho*_measurement(_index_temp)+virial;
      }

      else {
        virial = 1/(3*_volume)*virial + _ptail*_rho;
        _measurement(_index_pressure) = _rho*_temp+virial;
      }
...
// TO BE FIXED IN EXERCISE 6
 if (_measure_magnet){
    ...
    _measurement(_index_magnet) = magnetization/_npart; //saves magnetization per spin 
...
// TO BE FIXED IN EXERCISE 6
  if(_measure_cv){
    ...
    double s_i, s_j;
      ...
      _measurement(_index_cv) = pow(H_temp,2);  //saves H^2
  }
  // SUSCEPTIBILITY ////////////////////////////////////////////////////////////          
// TO BE FIXED IN EXERCISE 6
  ...
  _measurement(_index_chi)=(_beta*((pow(chi_temp,2))/_npart));  //saves susceptibility per particle (with h=0)
}
  //GOFR
//TO BE FIXED IN EXERCISE 7
  ...
  if(_measure_gofr){
    for (int k=0; k<_n_bins; k++){
      ...
      double r_up = (k+1)*_bin_size; //outer radius of the k-th spherical shell
      double r_down = k*_bin_size;  //inner radius of the k-th spherical shell
      double deltaV = 4.0/3.0*M_PI*(pow(r_up,3)-pow(r_down,3)); //volume f the spherical shell
      _measurement(_index_gofr + k) = histo[k]/double (_rho*_npart*deltaV); //normalization and storage in _measurement(_index_gofr+k)
    } 
  }
...
void System :: averages(int blk){
  ...
  if(_measure_cv and _measure_tenergy)
    _average(_index_cv)=pow(_beta,2)*(_average(_index_cv)-pow(_average(_index_tenergy)*_npart,2))/double(_npart);
  ...
  if (_measure_tenergy){
    ...
    // Save only the final progressive value
    if (blk == _nblocks and _sim_type == 2) { //_sym_type ==2 --> Simulation uses Metropolis

        coutf.open("../OUTPUT/Metropolis/total_energy.dat", ios::app);
    ...
    else if (blk == _nblocks and _sim_type == 3){ //_sym_type ==3 --> Simulation uses Gibbs

      coutf.open("../OUTPUT/Gibbs/total_energy.dat", ios::app);
  ...
  if(_measure_gofr){
    ...
    for(int k=0; k<_n_bins; k++){
      ...
      double bin_center = (k+0.5)*_bin_size; //center of each bin of the histogram
  ...
  // TO BE FIXED IN EXERCISE 6
   ...
   if(_measure_magnet){
    ...
    if (blk == _nblocks and _sim_type == 2) { //_sym_type ==2 --> Simulation uses Metropolis
        ...
        coutf.open("../OUTPUT/Metropolis/magnetization.dat", ios::app);
      ...
      coutf.open("../OUTPUT/Gibbs/magnetization.dat", ios::app);
  ...
  if(_measure_cv){
    ...
    if (blk == _nblocks and _sim_type ==2) {  //_sym_type ==2 --> Simulation uses Metropolis
        ...
        coutf.open("../OUTPUT/Metropolis/specific_heat.dat", ios::app);
    ...
    else if (blk == _nblocks and _sim_type == 3){ //_sym_type ==2 --> Simulation uses Gibbs

        coutf.open("../OUTPUT/Gibbs/specific_heat.dat", ios::app);
  ...
  // TO BE FIXED IN EXERCISE 6
    ...
    if (blk == _nblocks and _sim_type == 2) { //_sym_type ==2 --> Simulation uses Metropolis
        ...
        coutf.open("../OUTPUT/Metropolis/susceptibility.dat", ios::app);
        ...
        coutf.open("../OUTPUT/Gibbs/susceptibility.dat", ios::app);
