...
private:
  ...
  int _sim_type;        // Type of simulation (e.g., Lennard-Jones, Ising) (sim_type = 0 significa che facciamo dinamica molecolare)
  ...
  double _J, _H;        // Parameters for the Ising Hamiltonian (in questo es non lo uso)
...
public: // Function declarations
  ...
  double error(double acc, double acc2, int blk); // Compute error (con il data blocking)
  void move(int part);        // Move a particle (in questo es non lo uso)
  ...
  int pbc(int i);             // Apply periodic boundary conditions for spins (in questo es non lo uso)
  void Verlet();              // Perform Verlet integration step
  double Force(int i, int dim); // Calculate force on a particle along a dimension
  double Boltzmann(int i, bool xnew); // Calculate Boltzmann factor for Metropolis acceptance (in questo es non lo uso)
