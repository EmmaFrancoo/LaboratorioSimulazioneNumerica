...
void System :: move(int i){ // Propose a MC move for particle i
  if(_sim_type == 3){ //Gibbs sampler for Ising
    ...
    JHsum_spin = _J*(_particle(this->pbc(i-1)).getspin() + _particle(this->pbc(i+1)).getspin() ) + _H; //campo sentito dallo spin i-esimo dovuto ai suoi primi vicini ed al campo esterno H
    p_up = 1./(1+exp(-2*_beta*JHsum_spin)); //probabilità che lo spin i-esimo sia up dovuto allo stato dei suoi vicini e del campo H
    if(_rnd.Rannyu()< p_up) 
...
void System :: initialize(){ // Initialize the System object according to the content of the input files in the ../INPUT/ directory
  ...
  while ( !input.eof() ){ 
    input >> property; 
    if( property == "SIMULATION_TYPE" ){ //se la proprietà è "Simulation type"
      input >> _sim_type; //allora mette il numero che legge affianco a "Simulation type" in _sim_type. 
      if(_sim_type > 1){ 
        input >> _J; 
        input >> _H; 
      }
      if(_sim_type > 3){ //controllo sul numero scritto di fianco a "Simulation Type" nell'input.
      ...
      for(int i=0; i<_npart; i++){ //loop per inizializzare le particelle
      ...
      for(int i=0; i<_ndim; i++) _side(i) = side; //loop che mette in ogni componente di side (ovvero lato lungo x,y,z) il valore del lato 
      _halfside=0.5*_side; //loop che mette in ogni componente di halfside il valore di metà del lato 
      coutf << "SIDE= ";
      ...
      input >> _r_cut; //setta il raggio di cutoff
      coutf << "R_CUT= " << _r_cut << endl;
    } else if( property == "DELTA" ){
      input >> delta; //setta il passo delta
      coutf << "DELTA= " << delta << endl;
      _delta = delta;
    } else if( property == "NBLOCKS" ){
      input >> _nblocks; //setta il numero di blocchi
      coutf << "NBLOCKS= " << _nblocks << endl;
    } else if( property == "NSTEPS" ){
      input >> _nsteps; //setta il numero di step per blocco
      coutf << "NSTEPS= " << _nsteps << endl;
    } else if( property == "ENDINPUT" ){
      coutf << "Reading input completed!" << endl; //fine setting proprietà del sistema da file di input
      break;
...
void System :: initialize_velocities(){ 
  if(_restart and _sim_type==0){
    ...
    sumv.zeros();
    for (int i=0; i<_npart; i++){
      vx(i) = _rnd.Gauss(0.,sqrt(_temp)); //setta velocità lungo x data dalla distribuzione di Maxwell-Boltzamnn
      vy(i) = _rnd.Gauss(0.,sqrt(_temp));
      vz(i) = _rnd.Gauss(0.,sqrt(_temp));
      sumv(0) += vx(i); //calcolo la velocità di drift
      sumv(1) += vy(i);
      sumv(2) += vz(i);
    }
...
void System :: read_configuration(){
  ...
  if(cinf.is_open()){
    ...
    int ncoord;
    cinf >> ncoord;
    if (ncoord != _npart){ //controlla che il numero di coordinate nei due files sia lo stesso
    ...
    for(int i=0; i<_npart; i++){
      ...
      _particle(i).setposition(0, this->pbc(_side(0)*x, 0)); //per ogni particella i-esima setta la posizione considerando le condizioni al contorno
...
// TO BE FIXED IN EXERCISE 6
 if (_measure_magnet){
    ...
    _measurement(_index_magnet) = magnetization/_npart; //salva la magnetizzazione per spin
...
// TO BE FIXED IN EXERCISE 6
  if(_measure_cv){
    ...
    double s_i, s_j;
      ...
      _measurement(_index_cv) = pow(H_temp,2);
  }
...
// TO BE FIXED IN EXERCISE 6
  ...
  _measurement(_index_chi)=(_beta*((pow(chi_temp,2))/_npart));
}
...
void System :: averages(int blk){
  ...
  if (_measure_tenergy){
    ...
    if (blk == _nblocks and _sim_type == 2) {
    ...
    else if (blk == _nblocks and _sim_type == 3){
